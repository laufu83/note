...
export const AuthController = {
  async register(
   ...
   // 1. 滑块验证码校验
    ...
    try {
      ...
      ) VALUES($1,$2,$3,'inactive','user',$4,$5,$6,$6) RETURNING id`,
  ...
  async login(env: Env, body: { username: string; password: string,captchaToken: string }) {
   ...
   const tokenKey = `img:token:${captchaToken}`;
    ...
    if (emailReg.test(username)) {
      // 输入是邮箱，按邮箱查询
      sql = `SELECT id,password_hash,status,role,is_frozen FROM users WHERE email=$1 AND deleted=false`;
    } else {
      // 输入是用户名，按用户名查询
      sql = `SELECT id,password_hash,status,role,is_frozen FROM users WHERE username=$1 AND deleted=false`;
    }
  ...
  async resendActivateMail(env: Env, body: { email: string }) {
    ...
    await pool.query(
      `UPDATE users SET activate_token=$1,activate_expire=$2 WHERE email=$3`,
      [newToken, expire, body.email],
    );
  ...
  async refreshToken(env: Env, body: { refreshToken: string }) {
    ...
    try {
      ...
      // 携带最新角色签发新accessToken
      const newAccess = await signAccessToken(uid, role, env);
  ...
  async resetPwd(env: Env, body: { token: string; newPwd: string }) {
    ...
    // 清除当前重置token
    await pool.query(`DELETE FROM user_reset_token WHERE reset_token=$1`, [
      token,
    ]);
    ...
    await pool.query(`DELETE FROM user_refresh_token WHERE user_id=$1`, [
      userId,
    ]);
...
async activateChangeEmail(env: Env, search?: URLSearchParams) {
  ...
  const activateRes = await pool.query(`
    SELECT user_id, new_email FROM user_email_activate
    WHERE activate_token = $1 AND expired_at > $2
  `, [token, now]);
  ...
  const { user_id, new_email } = activateRes.rows[0];

  // 事务：更新用户邮箱 + 删除激活记录
  await pool.query("BEGIN");
  try {
    console.log(new_email,user_id)
    await pool.query(
      `UPDATE users SET email = $1, updated_at = NOW() WHERE id = $2 AND deleted = false`,
      [new_email, user_id]
    );
    await pool.query(`DELETE FROM user_email_activate WHERE activate_token = $1`, [token]);
